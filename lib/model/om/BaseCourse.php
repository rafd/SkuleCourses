...
abstract class BaseCourse extends BaseObject  implements Persistent {
  ...
  const PEER = 'CoursePeer';
	...
	/**
	 * @var        array CourseSection[] Collection to store aggregation of CourseSection objects.
	 */
	protected $collCourseSections;

	/**
	 * @var        Criteria The criteria used to select the current contents of collCourseSections.
	 */
	private $lastCourseSectionCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collCourseSections = null;
			$this->lastCourseSectionCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collCourseSections !== null) {
				foreach ($this->collCourseSections as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getCourseSections() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addCourseSection($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collCourseSections collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addCourseSections()
	 */
	public function clearCourseSections()
	{
		$this->collCourseSections = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collCourseSections collection (array).
	 *
	 * By default this just sets the collCourseSections collection to an empty array (like clearcollCourseSections());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initCourseSections()
	{
		$this->collCourseSections = array();
	}

	/**
	 * Gets an array of CourseSection objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Course has previously been saved, it will retrieve
	 * related CourseSections from storage. If this Course is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array CourseSection[]
	 * @throws     PropelException
	 */
	public function getCourseSections($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CoursePeer::DATABASE_NAME);
		}
		...
		if ($this->collCourseSections === null) {
			if ($this->isNew()) {
			   $this->collCourseSections = array();
				...
				$criteria->add(CourseSectionPeer::COURSE_ID, $this->id);

				CourseSectionPeer::addSelectColumns($criteria);
				$this->collCourseSections = CourseSectionPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				CourseSectionPeer::addSelectColumns($criteria);
				if (!isset($this->lastCourseSectionCriteria) || !$this->lastCourseSectionCriteria->equals($criteria)) {
					$this->collCourseSections = CourseSectionPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastCourseSectionCriteria = $criteria;
		return $this->collCourseSections;
	}

	/**
	 * Returns the number of related CourseSection objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related CourseSection objects.
	 * @throws     PropelException
	 */
	public function countCourseSections(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CoursePeer::DATABASE_NAME);
		} else {
		...
		if ($this->collCourseSections === null) {
			if ($this->isNew()) {
				...
				$count = CourseSectionPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastCourseSectionCriteria) || !$this->lastCourseSectionCriteria->equals($criteria)) {
					...
					$count = count($this->collCourseSections);
				}
			} else {
				$count = count($this->collCourseSections);
			}
	...
	/**
	 * Method called to associate a CourseSection object to this object
	 * through the CourseSection foreign key attribute.
	 *
	 * @param      CourseSection $l CourseSection
	 * @return     void
	 * @throws     PropelException
	 */
	public function addCourseSection(CourseSection $l)
	{
		if ($this->collCourseSections === null) {
			$this->initCourseSections();
		}
		if (!in_array($l, $this->collCourseSections, true)) { // only add it if the **same** object is not already associated
			array_push($this->collCourseSections, $l);
			$l->setCourse($this);
		}
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Course has previously been saved, it will retrieve
	 * related CourseTypeAssociations from storage. If this Course is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collCourseTypeAssociations === null) {
			if ($this->isNew()) {
			   $this->collCourseTypeAssociations = array();
				...
				$criteria->add(CourseTypeAssociationPeer::COURSE_ID, $this->id);
	 ...
	 */
	public function getCourseTypeAssociationsJoinEnumItem($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CoursePeer::DATABASE_NAME);
		}
		...
		if ($this->collCourseTypeAssociations === null) {
			if ($this->isNew()) {
				...
				$this->collCourseTypeAssociations = CourseTypeAssociationPeer::doSelectJoinEnumItem($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * actually need in Course.
	 */
	public function getCourseDisciplineAssociationsJoinDiscipline($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(CoursePeer::DATABASE_NAME);
		}
		...
		if ($this->collCourseDisciplineAssociations === null) {
			if ($this->isNew()) {
				...
				$this->collCourseDisciplineAssociations = CourseDisciplineAssociationPeer::doSelectJoinDiscipline($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collCourseSections) {
				foreach ((array) $this->collCourseSections as $o) {
					$o->clearAllReferences($deep);
				}
			}
