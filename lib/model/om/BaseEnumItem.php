...
abstract class BaseEnumItem extends BaseObject  implements Persistent {
  ...
  const PEER = 'EnumItemPeer';
	 ...
	 * related CourseInstructorAssociations from storage. If this EnumItem is new, it will return
		...
		if ($this->collCourseInstructorAssociations === null) {
			if ($this->isNew()) {
			   $this->collCourseInstructorAssociations = array();
				...
				$criteria->add(CourseInstructorAssociationPeer::DISCIPLINE_ID, $this->id);
	 ...
	 * actually need in EnumItem.
	 */
	public function getCourseInstructorAssociationsJoinCourse($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EnumItemPeer::DATABASE_NAME);
		}
		...
		if ($this->collCourseInstructorAssociations === null) {
			if ($this->isNew()) {
				...
				$this->collCourseInstructorAssociations = CourseInstructorAssociationPeer::doSelectJoinCourse($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
